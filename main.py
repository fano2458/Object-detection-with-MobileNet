...
from rdd_utils.dataset import RDDDataset
...
# DATA_ROOT = 'datasets/open-images-bus-trucks/'
...
# train_ds = OpenDataset(trn_df, DATA_ROOT + 'images')
# test_ds = OpenDataset(val_df, DATA_ROOT + "images")

# train_loader = DataLoader(train_ds, batch_size=32, collate_fn=train_ds.collate_fn, drop_last=True, shuffle=True, pin_memory=False)
# test_loader = DataLoader(test_ds, batch_size=32, collate_fn=test_ds.collate_fn, drop_last=True, shuffle=False, pin_memory=False)

train_dataset = RDDDataset('datasets/rdd/Japan', split='train', augment=None)
test_dataset = RDDDataset('datasets/rdd/Japan', split='test', augment=None)

train_loader = DataLoader(train_dataset, batch_size=32, collate_fn=train_dataset.collate_fn, shuffle=True, pin_memory=False)
test_loader = DataLoader(test_dataset, batch_size=32, collate_fn=test_dataset.collate_fn, shuffle=False, pin_memory=False)
num_classes = 4
...
criterion = MultiBoxLoss(priors_cxcy=model.priors_cxcy, device='cuda')
