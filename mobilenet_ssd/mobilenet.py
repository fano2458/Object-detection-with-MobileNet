...
#     def __init__(self, kernel_size, in_planes, exp_planes, out_planes, act, se, stride):
#         super(BBlock, self).__init__()

#         self.se = se
#         self.se_block = SE(exp_planes, 4)
#         # self.short = stride == 1 and in_planes == out_planes

#         self.pointwise1 = nn.Sequential(
#             nn.Conv2d(in_planes, exp_planes, kernel_size=1, stride=1, bias=False),
#             nn.BatchNorm2d(exp_planes),
#             act(True)
#         )

#         self.depthwise1 = nn.Sequential(
#             nn.Conv2d(exp_planes, exp_planes, kernel_size==kernel_size, padding=(kernel_size+1)//2, groups=exp_planes, stride=stride, bias=False),
#             nn.BatchNorm2d(exp_planes),
#             act(True)
#         )

#         self.pointwise2 = nn.Sequential(
#             nn.Conv2d(exp_planes, out_planes, kernel_size=1, stride=1, bias=False),
...
#         x = self.pointwise1(x)
#         x = self.depthwise1(x)
#         x = self.se_block(x) if self.se else x
#         x = self.pointwise2(x)
#         # x = x + identity if self.short else x

#         return x


# class BBLocks(nn.Module):
#     def __init__(self):
#         super(BBLocks, self).__init__()
#         self.blocks = nn.Sequential(
#             BBlock(kernel_size=3, in_planes=16, exp_planes=16, out_planes=16, act=nn.ReLU, se=False, stride=1),
#             BBlock(kernel_size=3, in_planes=16, exp_planes=64, out_planes=24, act=nn.ReLU, se=False, stride=1),
#             BBlock(kernel_size=3, in_planes=24, exp_planes=72, out_planes=24, act=nn.ReLU, se=False, stride=1),
#             BBlock(kernel_size=5, in_planes=24, exp_planes=72, out_planes=40, act=nn.ReLU, se=True, stride=1),
#             BBlock(kernel_size=5, in_planes=40, exp_planes=120, out_planes=40, act=nn.ReLU, se=True, stride=1),
#             BBlock(kernel_size=5, in_planes=40, exp_planes=120, out_planes=40, act=nn.ReLU, se=True, stride=1),
#             BBlock(kernel_size=3, in_planes=40, exp_planes=240, out_planes=80, act=nn.Hardswish, se=False, stride=2),
#             BBlock(kernel_size=3, in_planes=80, exp_planes=200, out_planes=80, act=nn.Hardswish, se=False, stride=1),
#             BBlock(kernel_size=3, in_planes=80, exp_planes=184, out_planes=80, act=nn.Hardswish, se=False, stride=1),
#             BBlock(kernel_size=3, in_planes=80, exp_planes=184, out_planes=80, act=nn.Hardswish, se=False, stride=1),
#             BBlock(kernel_size=3, in_planes=80, exp_planes=480, out_planes=112, act=nn.Hardswish, se=True, stride=1),
#             BBlock(kernel_size=3, in_planes=112, exp_planes=672, out_planes=112, act=nn.Hardswish, se=True, stride=1),
#             BBlock(kernel_size=5, in_planes=112, exp_planes=672, out_planes=160, act=nn.Hardswish, se=True, stride=2),
#             BBlock(kernel_size=5, in_planes=160, exp_planes=960, out_planes=160, act=nn.Hardswish, se=True, stride=1),
#             BBlock(kernel_size=5, in_planes=160, exp_planes=960, out_planes=160, act=nn.Hardswish, se=True, stride=1)
#         ) 
...
#             nn.Conv2d(160, 960, kernel_size=1, padding=0, stride=1, bias=False),
#             nn.BatchNorm2d(960),
#             nn.Hardswish(True)
#         )
...
#             nn.AdaptiveAvgPool2d(1),
#             nn.Conv2d(960, 1280, kernel_size=1, padding=0, stride=1, bias=False),
#             nn.Hardswish(True)
#         )

#         self.conv4 = nn.Conv2d(1280, class_num, kernel_size=1, padding=0, stride=1, bias=False)
